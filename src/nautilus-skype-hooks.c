 ...
 * Implements connection handling and C interface for the Skype hook socket.
 *
...
typedef struct {
  SkypeUpdateHook hook;
  gpointer ud;
} HookData;

static gboolean
try_to_connect(NautilusSkypeHookserv *hookserv);
...
handle_hook_server_input(GIOChannel *chan,
  ...
  while (1) {
    ...
    /* read the command name */
    {
      ...
      hookserv->hhsi.command_name = skype_client_util_desanitize(line);
      g_free(line);
    }
	...
	parse_result =
	  skype_client_util_command_parse_arg(line,
						hookserv->hhsi.command_args);
	g_free(line);
...
static void
watch_killer(NautilusSkypeHookserv *hookserv) {
...
gboolean nautilus_skype_hooks_force_reconnect(NautilusSkypeHookserv *hookserv) {
  debug_enter();
...
gboolean
nautilus_skype_hooks_is_connected(NautilusSkypeHookserv *hookserv) {
  return hookserv->connected;
}

void
nautilus_skype_hooks_setup(NautilusSkypeHookserv *hookserv) {
...
void nautilus_skype_hooks_add(NautilusSkypeHookserv *ndhs,
...
void
nautilus_skype_hooks_start(NautilusSkypeHookserv *hookserv) {
  try_to_connect(hookserv);
}
