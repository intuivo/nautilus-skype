 ...
 * Implements connection handling and C interface for the Skype command socket.
 *
...
/*
  ...
  finish_file info command is in nautilus_skype,
...
typedef struct {
  SkypeCommandClientConnectionAttemptHook h;
  gpointer ud;
} SkypeCommandClientConnectionAttempt;

typedef struct {
  SkypeGeneralCommand *dgc;
  GHashTable *response;
} SkypeGeneralCommandResponse;

static gboolean
on_connect(SkypeCommandClient *dcc) {
  g_hook_list_invoke(&(dcc->onconnect_hooklist), FALSE);
  return FALSE;
}

static gboolean
on_disconnect(SkypeCommandClient *dcc) {
...
on_connection_attempt(ConnectionAttempt *ca) {
  ...
  for (ll = ca->dcc->ca_hooklist; ll != NULL; ll = g_list_next(ll)) {
    SkypeCommandClientConnectionAttempt *dccca =
      (SkypeCommandClientConnectionAttempt *)(ll->data);
  ...
  while (1) {
    ...
    *(line+term_pos) = '\0';
    ...
    if (strcmp("done", line) == 0) {
      ...
      parse_result = skype_client_util_command_parse_arg(line, return_table);
      g_free(line);
...
/*
  sends a command to the skype server
...
#define WRITE_OR_DIE_SANI(s,l) {					\
    gchar *sani_s;							\
    sani_s = skype_client_util_sanitize(s);				\
...
#undef WRITE_OR_DIE_SANI
  ...
  if (iostat == G_IO_STATUS_ERROR) {
    ...
    g_assert(line == NULL);
    g_set_error(err,
		g_quark_from_static_string("skype command connection timed out"),
		0,
		"skype command connection timed out");
    return NULL;
  }
  else if (iostat == G_IO_STATUS_EOF) {
    g_assert(line == NULL);
    g_set_error(err,
		g_quark_from_static_string("skype command connection closed"),
		0,
		"skype command connection closed");
    return NULL;
  }
...
exit:
  dficr = g_new0(SkypeFileInfoCommandResponse, 1);
  dficr->dfic = dfic;
  ...
  g_idle_add((GSourceFunc) nautilus_skype_finish_file_info_command, dficr);
...
static gboolean
finish_general_command(SkypeGeneralCommandResponse *dgcr) {
...
static void
do_general_command(GIOChannel *chan, SkypeGeneralCommand *dcac,
		   GError **gerr) {
  GError *tmp_gerr = NULL;
  GHashTable *response;
  ...
  {
    SkypeGeneralCommandResponse *dgcr = g_new0(SkypeGeneralCommandResponse, 1);
    dgcr->dgc = dcac;
...
static gpointer
skype_command_client_thread(SkypeCommandClient *data);

static void
end_request(SkypeCommand *dc) {
  if ((gpointer (*)(SkypeCommandClient *data)) dc != &skype_command_client_thread) {
    switch (dc->request_type) {
    case GET_FILE_INFO: {
      SkypeFileInfoCommand *dfic = (SkypeFileInfoCommand *) dc;
      SkypeFileInfoCommandResponse *dficr = g_new0(SkypeFileInfoCommandResponse, 1);
      dficr->dfic = dfic;
    ...
    case GENERAL_COMMAND: {
      SkypeGeneralCommand *dgc = (SkypeGeneralCommand *) dc;
      SkypeGeneralCommandResponse *dgcr = g_new0(SkypeGeneralCommandResponse, 1);
      dgcr->dgc = dgc;
      dgcr->response = NULL;
      finish_general_command(dgcr);
...
/* thread safe */
gboolean
skype_command_client_is_connected(SkypeCommandClient *dcc) {
...
void skype_command_client_force_reconnect(SkypeCommandClient *dcc) {
  if (skype_command_client_is_connected(dcc) == TRUE) {
    debug("forcing command to reconnect");
    skype_command_client_request(dcc, (SkypeCommand *) &skype_command_client_thread);
  }
}
...
skype_command_client_add_connection_attempt_hook(SkypeCommandClient *dcc,
						   ...
						   gpointer ud) {
  SkypeCommandClientConnectionAttempt *newhook;

  debug("shouldn't be here...");
  
  newhook = g_new(SkypeCommandClientConnectionAttempt, 1);
  newhook->h = dhcch;
  newhook->ud = ud;
...
skype_command_client_start(SkypeCommandClient *dcc) {
  ...
  g_thread_create((gpointer (*)(gpointer data)) skype_command_client_thread,
		  dcc, FALSE, NULL);
}
...
void skype_command_client_send_simple_command(SkypeCommandClient *dcc, 
  ...
  dgc = g_new(SkypeGeneralCommand, 1);
  ...
  skype_command_client_request(dcc, (SkypeCommand *) dgc);
}
...
void skype_command_client_send_command(SkypeCommandClient *dcc, 
  ...
  dgc->command_args = g_hash_table_new_full((GHashFunc) g_str_hash,
					    ...
					    (GDestroyNotify) g_strfreev);
  /*
   * NB: The handler is called in the SkypeCommandClient Thread.  If you need
