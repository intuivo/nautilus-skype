...
for cmd in commands:
    ...
    formatted_commands += split[1].decode('ascii').replace(cmd, "`%s`" % cmd).replace("skype", "``skype``")
...
sys.stdout.write(sys.stdin.read().replace\
                     ...
                     ('@SYNOPSIS@', '| '+'\n| '.join(commands[cmd].__doc__.split('\n', 2)[1].decode('ascii').replace(cmd, "`%s`" % cmd).replace("skype", "``skype``") for cmd in commands)).replace\
