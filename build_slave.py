MASTER_ADDR = 'sonic.corp.skype.com'
MASTER_PORT = 80
...
class BuildController(SlaveController):
    ...
    def get_info_string(self):
        return "nautilus-skype"

    def generate_packages(self):
        if os.path.exists('/tmp/nautilus-skype-release.tar.gz'):
            assert self.system('rm -f /tmp/nautilus-skype-release.tar.gz') == 0

        # Source
        assert self.system('cp nautilus-skype-*.tar.bz2 /home/releng/result/packages') == 0

        # Get skype.py
        assert self.system('make skype') == 0
        assert self.system('cp /home/releng/nautilus-skype/skype /home/releng/result/packages/skype.py') == 0

        # Tar it up
        assert self.system('cd /home/releng/result/; tar czvf /tmp/nautilus-skype-release.tar.gz *') == 0

        rev = cmd('hg id -n'.split())

        # SCP it to sonic
        assert self.system('scp /tmp/nautilus-skype-release.tar.gz sonic:/var/www/builds/nautilus-skype/nautilus-skype-release-%s.tar.gz' % rev) == 0
    ...
    def generate_yum_repo(self, distro_upper, codenames, dist, archs, dist_name):
        ...
        files = os.listdir('/home/releng/result/fedora/pool')
        for f in files:
            if re.match(r'nautilus-skype-[0-9.-]*\.fedora\.(i386|x86_64)\.rpm', f):
